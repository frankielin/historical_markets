...
reg_dat[Rollout_Date!=0, .(unique_ids = uniqueN(town_state_id)), by = Year]
...
#################################
## Callaway-Sant'anna Balanced ##
#################################
## Here I maintain a balanced regression by keeping all time periods that are 
## within 10 +/- years of a transition, I hold the entire sample of
## untreated sample consistent throughout the entire time period
bal_reg_dat <- all_panel_dat
bal_reg_dat[Rollout_Year != 0 ,year_since_rollout:= Year - Rollout_Year]
bal_reg_dat <- bal_reg_dat[(Rollout_Year == 0) | (year_since_rollout >= -10) & (year_since_rollout <= -10)] 
...
bal_event_study <- aggte(bal_gt_out, type = "dynamic")
summary(bal_event_study)

ggdid(bal_event_study) + 
  labs(x = "Event Time (Years relative to treatment)", y = "ATT") +
  theme_minimal()
...
## Earlier groups show less power (effect)
## This is expected as these are typically smaller groups
bal_group_att <- aggte(bal_gt_out, type = "group")
summary(group_att)

## Calendar Year Analysis
## Earlier groups show less power (effect)
## This is expected as these are typically smaller groups
bal_calendar_att <- aggte(bal_gt_out, type = "calendar")
summary(calendar_att)

## Overall Effect
bal_overall_att <- aggte(bal_gt_out, type = "simple") 
summary(bal_overall_att)
